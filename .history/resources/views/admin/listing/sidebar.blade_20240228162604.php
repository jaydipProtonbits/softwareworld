...
<div class="bs-stepper-header">
  <div class="step" data-target="#account-details-vertical">
    <button data-create="{{ (!empty($current_route) ? $current_route == 'manage_listing.create' || $current_route == 'listing.create' : 0) }}"
  ...
  <div class="line"></div>
  <div class="step {{ empty($listing) ? 'menu_disabled' : '' }}" data-depend="Software Information" data-target="#personal-info-vertical">
    <button
    data-url="{{ (!empty($listing) && $listing->getMeta('is_information') == 1) ? (auth()->user()->is_admin() ? url('admin/listing/description/'.$listing->id) : url('vendor/listing/description/'.$listing->id)) : ''}}"
     type="button" {{ (!empty($listing) && $listing->getMeta('is_information') == 1) ? '' : 'disabled="disabled"'}} class="step-trigger">
      <span class="bs-stepper-circle">
  ...
  <div class="line"></div>
  <div class="step {{ empty($listing) ? 'menu_disabled' : '' }}" data-depend="Software Information" data-target="#social-links-vertical">
    <button
...
@if(!empty($listing) && $listing->status == 0 && auth()->user()->is_admin())
    ...
    <a href="javascript:void(0)" class="btn btn-primary status_change d-block mt-3" data-action="1" data-id="{{$listing->id??''}}">Approve</a>
    <a href="javascript:void(0)" class="btn btn-danger status_change d-block mt-3" data-action="0" data-id="{{$listing->id??''}}">Decline</a>
  @endif

        ...
        jQuery(document).ready(function($) {
            ...
            if (dt_user_table.length) {
                var dt_user = dt_user_table.DataTable({
                  ...
                  serverSide: true,
                  ajax: {
                    url: baseUrl + 'admin/users/UserManagement'
                  },
                  columns: [
                  ...
                  columnDefs: [{
                    ...
                    render: function render(data, type, full, meta) {
                      ...
                      var $state = states[stateNum],
                        $name = full['name'],
                        $pic = full['profile_pic'],
                        $initials = $name.match(/\b\w/g) || [],
                        $output;
                      $initials = (($initials.shift() || '') + ($initials.pop() || '')).toUpperCase();
                      $output = '<span class="avatar-initial rounded-circle bg-label-' + $state + '">' 
                      + (($pic != '') ? '<img src="'+$pic+'" class="rounded-circle">' : $initials)  +'</span>';

                      // Creates full output for row
                      var $row_output = '<div class="d-flex justify-content-start align-items-center user-name">' + '<div class="avatar-wrapper">' + '<div class="avatar avatar-sm me-3">' + $output + '</div>' + '</div>' + '<div class="d-flex flex-column">' + '<a href="users/'+full['id']+'/edit/" class="text-body text-truncate"><span class="fw-medium">' + $name + '</span></a>' + '</div>' + '</div>';
                      return $row_output;
                    }
                  }, {
                    // User email
                    ...
                    orderable: false,
                    render: function render(data, type, full, meta) {
                      return '<div class="d-inline-block text-nowrap">' + "<a class=\"btn font-dark btn-sm btn-icon edit-record\" href='users/"+full['id']+"/edit/' data-id=\"".concat(full['id'], "\" data-bs-toggle=\"\" data-bs-target=\"\"><i class=\"ti ti-edit\"></i></a>") + "<button class=\"btn btn-sm btn-icon delete-record\" data-id=\"".concat(full['id'], "\"><i class=\"ti ti-trash\"></i></button>") + '</div>';
                    }
                  }],
                  order: [[2, 'desc']],
                  ...
                  buttons: [{
                    ...
                    action: function ( e, dt, button, config ) {
                        window.location = baseUrl+'admin/users/create/';
                      }
                  }],
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts/layoutMaster', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/m6ek6c99z96m/public_html/laravel-dev.softwareworld.co/resources/views/admin/user/index.blade.php ENDPATH**/ ?>
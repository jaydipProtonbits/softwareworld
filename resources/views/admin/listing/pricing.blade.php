  ...
  <div class="col-12 mb-4">
    <div class="bs-stepper vertical wizard-vertical-icons-example mt-2">
      @include('admin.listing.sidebar')
        <form class="needs-validation1" action="{{route('savePrice'
          ...
          <div id="account-details-vertical" class="content active">
            ...
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <select class="form-control" name="pricing_type" id="pricing_type" required>
                  ...
                  @foreach (pricingType() as $key => $value)
                    <option value="{{$key}}" {{$key == $listing->getMeta('pricing_type') ? 'selected' : ''}}>{{$value}}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-md-6">
                ...
                <select class="form-control" name="pricing_currency" id="pricing_currency" required>
                  ...
                  @foreach (pricingCurrency() as $key => $value)
                    <option {{$key == $listing->getMeta('pricing_currency') ? 'selected' : ''}} value="{{$key}}">{{$value}}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-12">
                ...
                <label class="f-big">
                    <input type="radio" class="form-check-label" {{1 == $listing->getMeta('is_free_version') ? 'checked' : ''}} name="is_free_version" value="1"> YES
                </label>&nbsp;&nbsp;
                <label class="f-big">
                    <input type="radio" class="form-check-label" {{1 != $listing->getMeta('is_free_version') ? 'checked' : ''}} name="is_free_version" value="0"> No
                </label>
              </div>
              <div class="col-12">
                    <label class="form-label required-label d-block notice-label">Free trial available in your software?</label>
                    <label class="f-big"><input type="radio" {{1 == $listing->getMeta('is_free_trail') ? 'checked' : ''}} class="form-check-label" name="is_free_trail" value="1"> YES </label>&nbsp;&nbsp;
                    <label class="f-big"><input type="radio" {{1 != $listing->getMeta('is_free_trail') ? 'checked' : ''}} class="form-check-label" name="is_free_trail" value="0"> No</label>
              </div>
                <div class="row {{$listing->getMeta('is_free_trail') == 1 ? '' : 'd-none'}} free_trail">
                    <div class="col-md-6">
                        <label class="form-label d-block notice-label">Do customer need to provide Credit Card details for trial?</label>
                        <label class="f-big">
                            <input type="radio" class="form-check-label" {{1 == $listing->getMeta('cc_for_trail_period') ? 'checked' : ''}} name="cc_for_trail_period" value="1"> YES
                        </label>&nbsp;&nbsp;
                        <label class="f-big">
                            <input type="radio" class="form-check-label" {{1 != $listing->getMeta('cc_for_trail_period') ? 'checked' : ''}} name="cc_for_trail_period" value="0"> No
                        </label>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="form-control" name="trail_duration" id="trail_duration">
                          @foreach (pricingTrail() as $key => $value)
                            <option value="{{$key}}" {{$key == $listing->getMeta('trail_duration') ? 'selected' : ''}}>{{$value}}</option>
                          @endforeach
                        </select>
                    </div>
             ...
             <div class="col-md-12">
                ...
                <select class="form-control" name="payment_frequency[]" id="payment_frequency" multiple>
                    {{--<option></option>--}}
                  @foreach (pricingFrequency() as $key => $value)
                    <option value="{{$value}}" {{ in_array($value, \Arr::wrap($listing->getMeta('payment_frequency')))  ? 'selected' : ''}}>{{$value}}</option>
                  @endforeach
                </select>
              </div>
             ...
             <div class="col-md-12">
                ...
                <textarea name="pricing_details" id="pricing_details" rows="5" placeholder="Enter description" class="form-control pricing_detail_desc" maxlength="500" cols="30">{{$listing->getMeta('pricing_details')}}</textarea>
              </div>
            ...
            <div class="row mt-5">
                ...
                <div class="col-12">
                    ...
                    <label class="f-big">
                        <input type="radio" class="form-check-label" {{1 == $listing->getMeta('is_price_plan') ? 'checked' : ''}} name="is_price_plan" value="1"> YES
                    </label>&nbsp;&nbsp;
                    <label class="f-big">
                        <input type="radio" class="form-check-label" {{1 != $listing->getMeta('is_price_plan') ? 'checked' : ''}} name="is_price_plan" value="0"> No
                    </label>
                </div>
                ...
                <div class="col-12">
                                ...
                                @foreach ($packages as $key => $package)
                                    ...
                                    <div class="package-box ">
                                        <div class="form-group">
                                            <div class="package-number">Package #<span class="package-digit">{{$loop->iteration}}</span>
                                                <span class="remove-package" data-listing-id="{{ $listing->id }}" data-id="{{ $loop->iteration }}">Remove</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-6 ps-0">
                                                <div class="input text">
                                                    <label for="package{{$loop->iteration}}Name" class="form-label">Package Name</label>
                                                    <input name="package[{{$loop->iteration}}][name]" placeholder="Enter package name" class="form-control package_name unique" maxlength="20" type="text" value="{{$package['name']}}" id="package{{$loop->iteration}}Name" required></div>
                                            </div>
                                            <div class="form-group  col-md-6">
                                                <div class="input text"><label for="package{{$loop->iteration}}Price" class="form-label">Price ($)</label><input name="package[{{$loop->iteration}}][price]" class="form-control package_price" autocomplete="off" maxlength="8" type="text" value="{{$package['price']}}" id="package{{$loop->iteration}}Price"></div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="form-group  col-md-6 packageunits">
                                                <label for="package{{$loop->iteration}}Units" class="form-label">Units</label>
                                                <select name="package[{{$loop->iteration}}][units]" class="form-control" id="package{{$loop->iteration}}Units">
                                                    <option value="">Please Select</option>
                                                    <option {{$package['units'] == 'one-time' ? 'selected' : ''}} value="one-time">One-time</option>
                                                    <option {{$package['units'] == 'year' ? 'selected' : ''}} value="year">Per Year</option>
                                                    <option {{$package['units'] == 'month' ? 'selected' : ''}} value="month">Per Month</option>
                                                    <option {{$package['units'] == 'day' ? 'selected' : ''}} value="day">Per Day</option>
                                                    <option {{$package['units'] == 'hour' ? 'selected' : ''}} value="hour">Per Hour</option>
                                                    <option {{$package['units'] == 'user' ? 'selected' : ''}} value="user">Per User</option>
                                                    <option {{$package['units'] == 'feautre' ? 'selected' : ''}} value="feautre">Per Feautre</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group clearfix">
                                            <label for="package{{$loop->iteration}}Description" class="form-label">Description</label>
                                            <textarea name="package[{{$loop->iteration}}][description]" rows="3" placeholder="Enter description" class="form-control package_description" maxlength="250" cols="30" id="package{{$loop->iteration}}Description">{{$package['description']}}</textarea>
                                        </div>
                                        <div class="strengthlayout clear form-group overflow">
                                            <div class="input text strengthlayout-area">
                                                ...
                                                @if (!empty($package['features']))
                                                    ...
                                                    @foreach ($package['features'] as $k => $val)
                                                        <div class="features-box clearfix first">
                                                            <span class="remove-features">-</span>
                                                            <input name="package[{{ $p_l }}][features][{{$k}}]" class="form-control features" autocomplete="off" type="text" maxlength="150" value="{{$val}}">
                                                        </div>
                                                    @endforeach
                                                @else
                                                    <div class="features-box clearfix first">
                                                        <span class="remove-features">-</span>
                                                        <input name="package[{{$loop->iteration}}][features][1]" class="form-control features" autocomplete="off" type="text" maxlength="150">
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                    ...
                    <a href="javascript:" class="add_package float-end {{$listing->getMeta('is_price_plan') == 1 ? '' : 'd-none'}}">Add More Package</a>
                </div>
            ...
            <div class="row mt-5">
              <div class="col-6 text-start">
                <a href="{{ (auth()->user()->is_admin()) ? route('details',['destination',$listing->id]) : route('manage_details',['destination',$listing->id]) }}" type="reset" class="btn btn-label-default waves-effect">
  ...
  <script type="text/javascript">
    jQuery(document).ready(function($) {
        ...
        $(document).on('click', '.remove-package', function () {
          ...
          if (package_id)
          {
            let url = '{{ route('delete_package', ':id') }}';

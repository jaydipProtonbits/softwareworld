...
<link rel="stylesheet" href="{{asset('assets/css/progress-animation.css')}}" />
    ...
    <div id="" class="bs-stepper vertical wizard-vertical-icons-example mt-2">
      ...
      <div class="bs-stepper-content">
        @if(count($errors) > 0 )
          ...
          @endif
        <form id="c_profile_info" class="needs-validation1" method="post" action="{{route('save_service',$service->id)}}" enctype="multipart/form-data" novalidate>
            @csrf
                ...
                <div class="row g-3">
                  <div class="content-header">
                    ...
                    <div class="clearfix service-box-wraper">
                                        ...
                                        <div class="main-category col-md-6 no-padding">
                                            ...
                                            <div class="category-list">
                                                ...
                                                <ul class="category-ul" id="catInputUL">
                                                    {{--@foreach ($category->where('type_id',1)->where('parent_id',2) as $value)--}}
                                                  @foreach ($main_category as $value)
                                                        <li class="category-li"><a cat-id="{{$value->id}}" cat-slug="{{slugify($value->name)}}">{{$value->name}}</a></li>
                                                    @endforeach
                                                </ul>
                                            </div>
                                        </div>
                                ...
                                <div class="account-tab-form-spe clear overflow">
                                    <div class="form-group overflow">
                                        <div class="row no-padding">
                                            {{--@foreach ($category->where('type_id',1)->where('parent_id',2) as $cat)--}}
                                            @foreach ($main_category as $cat)
                                                <div class="form-group all-services padding-top col-md-4 mt-3 hide cat-id-{{$cat->id}}">
                                                    <input type="hidden" name="ServiceLine[{{$cat->id}}][category_id]" value="{{$cat->id}}" id="ServiceLine{{$cat->id}}UsersFocusCategoryId">
                                                    <input type="hidden" name="ServiceLine[{{$cat->id}}][is_parent]" value="{{$cat->id}}" id="ServiceLine0UsersFocusIsParent">
                                                    <input type="hidden" name="ServiceLine[{{$cat->id}}][slug]" value="{{slugify($cat->name)}}" id="ServiceLine{{$cat->id}}UsersFocusSlug">
                                                    <label for="">{{$cat->name}}</label>
                                                    <div class="input-group col-md-8">
                                                        <input name="ServiceLine[{{$cat->id}}][percentage]" id="{{slugify($cat->name)}}" class="form-control service-line-percentage" type="text" value="0">
                      ...
                      @foreach ($main_category as $value)
                        {{--@foreach ($category->where('type_id',1)->where('parent_id',2) as $value)--}}
                        <div id="focy-{{slugify($value->name)}}" class="{{slugify($value->name)}} focyparentdiv-{{$value->id}}  focyparentdiv col-xs-12 radious4 whitebg margin-top padding blackc border hide" >
                              ...
                              <div class="col-md-4 no-padding">
                                <div class="main-focus mb-3">
                                  ...
                                  <div class="focus-list">
                                    ...
                                    <ul class="focus-ul">
                                      @foreach ($category->where('parent_id',$value->id) as $val)
                                        <li class="focus-li"><a focus-id="{{$val->id}}" focus-slug="{{slugify($val->name)}}">{{$val->name}}</a></li>
                                      @endforeach
                                    </ul>
                                  </div>
                                </div>
                                ...
                                <div class="progress-bar-status account-tab-form-title ">Sub Categories of <div class="sub_focus">{{$value->name}}</div>
                                </div>
                              </div>
                              <div class="col-md-8">
                                  ...
                                  <div id="mobile_focus">
                                    <div class="account-tab-form-spe clear overflow">
                                      <div class="form-group overflow">
                                        <div class="row no-padding">
                                          @foreach ($category->where('parent_id',$value->id) as $val)
                                            <div class="form-group allfocy padding-top col-md-6 mb-2 no-padding hide focus-id-{{$val->id}}">
                                              <input type="hidden" name="data[MobileFocus][{{$value->id}}][{{$val->id}}][focus_id]" value="{{$val->id}}" id="MobileFocus20UsersFocusFocusId"><input type="hidden" name="data[MobileFocus][{{$value->id}}][{{$val->id}}][category_id]" value="{{$val->id}}" id="MobileFocus20UsersFocusCategoryId"><input type="hidden" name="data[MobileFocus][{{$value->id}}][{{$val->id}}][slug]" value="{{slugify($val->name)}}" id="MobileFocus20UsersFocusSlug"> <label for="{{slugify($val->name)}}">{{$val->name}}</label>
                                              <div class="input-group col-md-8">
                                                <input name="data[MobileFocus][{{$value->id}}][{{$val->id}}][percentage]" id="{{slugify($val->name)}}" class="form-control platform-percentage" type="text" value="0">
                                                <span class="input-group-text">%</span>
                                              </div>
                                            </div>
                                          @endforeach
                  ...
                  <div class="row mt-5">
                    <div class="col-6 text-start">
                      <a href="{{ (auth()->user()->is_admin()) ? route('service_step',['location',$service->id]) : route('manage_service_step',['location',$service->id]) }}" type="reset" class="btn btn-label-default waves-effect">

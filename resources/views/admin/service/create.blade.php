                ...
                <div class="row g-3">
                  ...
                  <div class="row mt-3">
                    ...
                    @foreach (getCertifications() as $key => $value)
                      <div class="col-sm-4 ">
                        <label class="form-label cms-check"> <input type="checkbox" class="form-check-input" name="meta[certifications][]" value="{{$key}}"> {{$value}}</label>
                      </div>
                    @endforeach
                  </div>
                  <div class="row">
      ...
      $(document).on('keyup', '.software-search-input', function (e) {
        ...
        } else
        {
          ...
          if ($(this).val() != '')
          {
            ...
            ajaxRevReq = $.ajax({
              url: "{{ route('find_service_listing') }}", //Don't forget to replace with your own post URL
              type: 'POST',
      ...
      if (content.hits.length > 0)
      {
        for (var i = 0; i < content.hits.length; ++i)
        {
          ...
          if(hit["logo"] == '')
            ...
            company_pic = "{{asset('assets/service')}}"+'/'+hit["logo"];
